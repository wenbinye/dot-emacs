...
(defvar xs-font-lock-keywords
  (append
   ...
   `(("'[\\]?.'" . font-lock-string-face)
     ,(list
       (concat (regexp-opt '("MODULE" "PACKAGE" "PREFIX") t)
               "\\s-*=")
       '(1 font-lock-keyword-face))
     ,(list (regexp-opt '("NO_OUTPUT" "NO_INIT" "IN" "OUTLIST"
                          "IN_OUTLIST" "OUT" "IN_OUT"))
            font-lock-type-face)
     ,(list
       (concat "^[ \t]*\\(" (regexp-opt
                            '("OUTPUT" "CODE" "INIT" "PREINIT" "SCOPE"
                              "INPUT" "C_ARGS" "PPCODE" "REQUIRE"
                              "CLEANUP" "POSTCALL" "BOOT" "VERSIONCHECK"
                              "PROTOTYPES" "PROTOTYPE" "ALIAS"
