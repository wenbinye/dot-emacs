...
;; Version: $Id: chinese-chess.el,v 1.1.1.1 2007-03-13 13:16:10 ywb Exp $
;; Keywords: 
...
(defun chinese-chess-board-latex ()
  (interactive)
  (let ((board chinese-chess-board))
    (with-current-buffer (get-buffer-create "*board-latex*")
      (erase-buffer)
      (insert
       "\\documentclass[12pt,a4paper]{article}\n"
       "\\usepackage{CJK,cchess,wasysym,palatino}\n"
       "\\linespread{1.6}\n"
       "\\begin{document}\n"
       "\\normalboard\n"
       "\\begin{position}\n")
      (let ((str ""))
        (dolist (i (number-sequence 1 chinese-chess-height))
          (dolist (m (append (aref board (1- i)) nil))
            (if m
                (setq str (concat str (format "\\piece{%c}{%d}{%S}\n"
                                              (+ ?a (chinese-chess-man-get m 'col) -1)
                                              (- 11 (chinese-chess-man-get m 'row))
                                              (let ((name (chinese-chess-man-get m 'name)))
                                                (cond ((eq name 'a) 'g)
                                                      ((eq name 'A) 'G)
                                                      (t name)))))))))
        (insert str))
