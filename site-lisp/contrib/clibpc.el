...
(defvar clibpc-rebuild-hook nil
  "Rebuild obarray hook")

(defvar clibpc-find-symbol-function 'clibpc-find-symbol
  "*Function call when push button in `help-buffer'")
...
(defun clibpc-describe-function (func)
  ...
  (let ((symbol (intern-soft func clibpc-obarray))
        (inhibit-read-only t))
    (when symbol
      (help-setup-xref (list 'clibpc-describe-function func)
                       (interactive-p))
      (save-excursion
        (with-output-to-temp-buffer (help-buffer)
          ...
          (with-current-buffer standard-output
            ...
            (make-text-button (progn (re-search-forward "`") (point))
                              ...
                              'action
                              (lambda (but)
                                (funcall clibpc-find-symbol-function but)))))))))

(defun clibpc-find-symbol (but)
