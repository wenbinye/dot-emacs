   ...
   (list (let ((method (gtk-perl-method-ap t))
           ...
           (gtk-perl-read-method
            (if method
              ...
              "Describe function: ")
            method))))
  (let (pkg method prototype real-pkg)
    (if (string-match func "::")
      ...
      (let ((methods (gtk-perl-get-inherit pkg "Methods")))
        (setq real-pkg (assoc method methods))
        (unless (and real-pkg (>= (cdr real-pkg) 0)
                     (setq real-pkg (car (aref gtk-perl-index (cdr real-pkg))))
                     (not (string= real-pkg pkg)))
          (setq real-pkg nil))))
    (if (setq prototype (gtk-perl-method-prototype pkg method))
        (with-output-to-temp-buffer (help-buffer)
          (princ (format "Arguments of %s%s:\n" func
                         (if real-pkg
                             (format "(inherit from %s::%s)" real-pkg method)
                           "")))
          ...
          (with-current-buffer standard-output
            (goto-char (point-min))
            (if (re-search-forward (if real-pkg
                                       "(inherit from \\(.*\\))"
                                     "Arguments of \\(.*\\):$")
                                   nil t)
